...
default_args = {"owner": "airflow", "start_date": datetime(2026, 1, 1), "retries": 1}
...
def task3_remover_sem_musica(**context):
    ...
    context["ti"].xcom_push(key="qtd_descartados", value=total_antes - len(df_limpo))
...
def task6_enriquecer_genero(**context):
    ...
    df.merge(df_genero, on="id_genero", how="left").to_csv(f"{CAMINHO_BASE}/task4.csv", sep=SEP, index=False)

def task7_media_avaliacao():
    df = pd.read_csv(f"{CAMINHO_BASE}/task4.csv", sep=SEP)
    media = df.groupby("nome_musica")["nota"].mean().reset_index().rename(columns={"nota": "media_avaliacao"})
    media["media_avaliacao"] = media["media_avaliacao"].round(2)
    media.sort_values("media_avaliacao", ascending=False).to_csv(f"{CAMINHO_BASE}/media_avaliacao.csv", sep=SEP, index=False)

def task8_total_artista():
    df = pd.read_csv(f"{CAMINHO_BASE}/task4.csv", sep=SEP)
    df.groupby("nome_artista").size().reset_index(name="total_musicas").sort_values("total_musicas", ascending=False).to_csv(f"{CAMINHO_BASE}/total_artista.csv", sep=SEP, index=False)

with DAG(dag_id="pipeline_stream_musical", default_args=default_args, schedule=None, catchup=False, tags=["ssa"]) as dag:
    task1 = BashOperator(task_id="task1_copiar_arquivo", bash_command="cp /opt/airflow/data/dados-stream.csv /opt/airflow/data/entrada.csv")
    ...
    task4 = SQLExecuteQueryOperator(task_id="task4_inserir_descartados", conn_id="postgres_default", sql="INSERT INTO descartados (total) VALUES ({{ ti.xcom_pull(task_ids='task3_remover_sem_musica', key='qtd_descartados') }});")
    task5 = SQLExecuteQueryOperator(task_id="task5_consultar_genero", conn_id="postgres_default", sql="SELECT id_genero, nome_genero FROM genero_musical;")
    ...
    task9 = BashOperator(task_id="task9_remover_entrada", bash_command="rm -f /opt/airflow/data/entrada.csv", trigger_rule="all_done")
